...
//페스워드 체크!
function checkPwd(isPrivate ,boardNo, menuCode, subMenuCode){
	...
	function useUserInput(data) {
		...
		if(data == isPrivate){
				...
				type: 'post',
				contentType: "application/x-www-form-urlencoded; charset=UTF-8",
				data: { 'menuCode' : menuCode, 'subMenuCode' : subMenuCode, 'isPrivate' : isPrivate, 'boardNo' : boardNo}, //필요한 데이터
				success: function(result) {
					if(result){						
						location.href=`/stuMenu/boardDetail?menuCode=${menuCode}&subMenuCode=${subMenuCode}&boardNo=${boardNo} `;
					}
					else{
						alert('일시적 오류가 발생했습니다.');
					}
